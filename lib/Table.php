<?php
	...
	*/
	...
	{		
		...
		*  @param If we should add a checkbox to each row, this is the name of the attribute to use a value
		*/
		public function __construct($data, $headers=null, $checkbox=null)
		{
			...
			foreach($data as $d)
			{
				...
				if ($checkbox != null)
				{
					$td = html('td', html('input', 'type:checkbox;name:'.$checkbox.'[];value:'.$d[$checkbox]),'class:'.$checkbox);
					$row->addChild($td);
				}
